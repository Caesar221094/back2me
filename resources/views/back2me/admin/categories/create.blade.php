...
@section('subtitle','Kategori membantu pengelompokan laporan')

@section('content')
<div class="card card-hover p-6 space-y-6">
    <h3 class="text-lg font-semibold text-slate-900">Tambah Kategori</h3>
    <form method="POST" action="{{ route('back2me.admin.categories.store') }}" class="space-y-4">
        @csrf
        <div>
            <label class="text-sm font-semibold text-slate-700">Nama</label>
            <input type="text" name="nama" value="{{ old('nama') }}" class="mt-1 w-full rounded-lg border-slate-200 focus:ring-indigo-500 focus:border-indigo-500" required>
            @error('nama')<p class="text-xs text-red-600 mt-1">{{ $message }}</p>@enderror
        </div>
        <div>
            <label class="text-sm font-semibold text-slate-700">Deskripsi (opsional)</label>
            <textarea name="deskripsi" rows="3" class="mt-1 w-full rounded-lg border-slate-200 focus:ring-indigo-500 focus:border-indigo-500">{{ old('deskripsi') }}</textarea>
